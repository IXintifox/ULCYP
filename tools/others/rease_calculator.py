...
class FAIRChemCalculator(Calculator):
    def __init__(
        ...
        Notes:
            - For ulcyp that require total charge and spin multiplicity (currently UMA ulcyp on omol mode), `charge`
        ...
        self.model = self.predictor.model.module.backbone
        # try to make sure no gradients are computed
        # self.model.direct_forces = True
        # self.model.regress_forces = False
        # self.model.regress_stress = False
        # self.model.cs_emb_grad = False
        # self.model.dataset_emb_grad = False
    ...
    def calculate(self, atoms: Atoms, properties: list[str], system_changes: list[str]) -> None:
        ...
        if len(atoms) == 1 and sum(atoms.pbc) == 0:
            ...
            self.results = {}
            self.results["embedding"] = features["node_embedding"]

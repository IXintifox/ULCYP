...
def select_representatives_grouped(cluster_labels, group_size=300):
    ...
    # Select the first molecule from each cluster as representative
    representatives = []
    ...
    # Group clusters, each group contains group_size clusters
    representatives_grouped = {}
    total_clusters = len(sorted_clusters)
    num_groups = (total_clusters + group_size - 1) // group_size  # Round up
...
def select_representatives_no_label(cluster_labels, total_representatives=300):
    # Select molecules based on labels (0/1)
    representatives_above = []  # Representatives of toxic molecules (label=1)
    ...
    # Process toxic and non-toxic molecules in each cluster separately
    for label in np.unique(cluster_labels):  # Number of clusters
        members = np.where(cluster_labels == label)[0]
        if len(members) > 0:
            # Group by label and put into toxic and non-toxic molecule lists separately
...
def scaffold_split_no_label(smiles_idx, smiles):
    ...
    # Calculate molecular fingerprints and perform clustering

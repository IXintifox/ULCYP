...
class ULCYP(nn.Module):
    def __init__(self, hidden_channels=256, out_channels=256,
                 ...
                 task_num=4, contrastive_tau=0.1, contrastive_weight=0.1,
                 gate_dropout=0.2, fusion_dropout=0.3, des_dropout=0.1):
        super(ULCYP, self).__init__()
        self.prior = prior
        self.task_num = task_num
        self.contrastive_tau = contrastive_tau
        self.contrastive_weight = contrastive_weight
        self.loss_tools = CalculateLoss(prior=self.prior, sub_model_num=sub_model_num,
                                       device=device, pu_task_num=task_num)
        self.fd_tools = DesFinger(hidden_dim=hidden_channels, dropout=des_dropout)
        ...
        self.gate_fusion = GatedFusion(dim_a=hidden_channels, dim_b=hidden_channels,
                                       dim_hidden=hidden_channels, dropout=gate_dropout)
        self.fusion = FusionMLP(input_dim=hidden_channels, hidden_dim=hidden_channels,
                               output_dim=hidden_channels, dropout=fusion_dropout)
    ...
    def logit_combine(self, features):
        sub_model_num = len(self.loss_tools.loop_special_task_idx)
        offset = (sub_model_num - 1) * self.task_num
        features = features[:, offset:]
        reshaped = features.view(features.size(0), sub_model_num, self.task_num)
    ...
    def get_loss(self, x, dataset_name, f=0, pu_task=False, training=False, cluster=1):
        ...
        con_loss = contrastive_loss_from_features(self.upstream_embedding_unimol,
                                                   self.upstream_embedding_gemini,
                                                   tau=self.contrastive_tau)
        pu_task_loss = self.loss_tools.get_pu_task_loss(output_features_cover, label, mask, cluster=cluster)

        full_loss = pu_task_loss + self.contrastive_weight * con_loss
        return_results = {
    ...
    def get_temperature(self, device=None, high_temp=2.0, low_temp=1.0, mid_temps=None):
        if device is None:
            device = self.device
        if mid_temps is None:
            mid_temps = [1.5, 3.0]
        cycle_unit = torch.tensor([low_temp] + mid_temps + [high_temp])
        sub_model_num = len(self.loss_tools.loop_special_task_idx)
        part_tasks = cycle_unit.repeat(sub_model_num)
...
class GatedFusion(nn.Module):
    def __init__(self, dim_a=256, dim_b=256, dim_hidden=256, dropout=0.2):
...
class DesFinger(nn.Module):
    def __init__(self, hidden_dim=256, dropout=0.1, device="cpu",
                 des_dim=217, uni_dim=768, gemini_dim=2048,
                 middle_dim=1536, middle_dim2=1024):
        super(DesFinger, self).__init__()
        self.des_dim = des_dim
        self.uni_dim = uni_dim
        self.gemini_dim = gemini_dim
        self.middle_dim = middle_dim
        self.middle_dim2 = middle_dim2
...
class CalculateLoss(TaskClass):
    def __init__(self, prior, sub_model_num=1, device="cuda", pu_task_num=4):
